 ...
 *
 * One possible solution is to use an Indexed Priority Queue (IPQ)
 * which can efficiently update and poll key-value pairs.
 *
 * This reduces the overall time complexity from O(E*logE) to 0 (E*logV)
 * since there can only be V (node, edge) pairs in the IPQ, making the update and poll operations 0(logV).
 */
import static java.lang.Math.*;
import java.util.*;

/**
 * Eager Prim'S MST Overview
 * Maintain a min Indexed Priority Queue (IPQ) of size V that sorts vertex-edge pairs (v, e)
 * based on the min edge cost of e.
 *
 * By default, all vertices v have a best value of (v, e) in the IPQ.
 *
 ...
 *
 *      Next, relax all edges of v while making sure not to relax any edge pointing
 *      to a node which has already been visited.
 *
 *      relaxing in this context refers to updating the entry for node v in the IPQ from (V, eold) to (V, enew)
 *      if the new edge from u → v has a lower cost than eold.
 *
