 ...
 * This will be used to determine the next node to visit and the edge.
 *
 * Start the algorithm on any node s.
 * Marks as visited and iterate over all edges of s, adding them to the PO.
 *
 * While the PQ is not empty and a MST has not been formed,
 *     dequeue the next cheapest edge from the PQ.
 *     If the dequeued edge is outdated (meaning the node it points to has already been visited)
 *         then skip it and poll again
 *
 *     Otherwise,
 ...
 * Do not add edges to the PQ which point to already visited
 *
 * An implementation of the lazy version of Prim's algorithm which relies on using a priority queue
...
public class AdjacencyListLazyPrims {
    ...
    // Inputs
    private final int n; // graph size
